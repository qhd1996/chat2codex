...
if (-not (Test-Path -LiteralPath $npmCli -PathType Leaf)) {
  ...
  if ($npmCommand) { $npmCli = $npmCommand.Source } else { Stop-Portable 'PORTABLE_NPM_MISSING' 'Install npm with Node.js and retry.' }
}
...
if (-not $DryRun) { Stop-Portable 'PORTABLE_EXECUTOR_NOT_READY' 'Use -DryRun until the transactional executor is included in the reviewed package.' }
$entrypoint = Join-Path (Split-Path $PSScriptRoot -Parent) 'dist\index.js'
