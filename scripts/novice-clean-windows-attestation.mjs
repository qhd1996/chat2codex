...
const protectedGlobalNpmRoot = path.resolve(read("--protected-global-npm-root") ?? "");
...
if (!process.env.CHAT2CODEX_NOVICE_ISOLATION || !path.isAbsolute(ownedRoot) || !path.isAbsolute(packageRoot) || !path.isAbsolute(codexBin) || !path.isAbsolute(productionRoot) || !path.isAbsolute(protectedRealCodexHome) || !path.isAbsolute(protectedGlobalNpmRoot)) throw new Error("Clean Windows attestation requires owned absolute inputs.");
...
const projectedGlobalNpmRoot = path.join(process.env.APPDATA ?? "", "npm", "node_modules");
if (!inside(environmentRoot, projectedGlobalNpmRoot) || overlaps(environmentRoot, protectedGlobalNpmRoot)) throw new Error("Projected or protected global npm root scope is invalid.");
const protectedGlobalNpmRootBefore = await snapshotTree(protectedGlobalNpmRoot);
if (await lstat(path.join(projectedGlobalNpmRoot, "chat2codex")).catch(() => null)) throw new Error("Prior projected global Chat2Codex package is present.");
...
function queryTaskDiagnostic(taskPath) { const result = spawnSync("schtasks.exe", ["/Query", "/TN", taskPath, "/V", "/FO", "LIST"], { encoding: "utf8", windowsHide: true }); return redactFailure("exit=" + result.status + "; " + result.stdout + "; " + result.stderr); }
async function readRedactedLogTail(file) { const source = await readFile(file, "utf8").catch(() => "missing"); return redactFailure(source); }
