...
describe("novice acceptance documentation and Windows CI", () => {
  ...
  test("runs the novice fast, native, 30-run, pack, extraction, and residual gates on Windows", async () => {
    ...
    expect(standardUserScript).toContain("Scheduled Task action exposes the password");
    expect(standardUserScript).not.toContain("-Password', $plain");
    ...
    for (const value of ["[IO.Directory]::CreateDirectory($OwnedRoot)", "$env:TEMP = $OwnedRoot", "$env:TMP = $OwnedRoot", "$env:USERPROFILE = $ProfilePath", "$env:HOME = $ProfilePath", "$env:C2C_NATIVE_LIFECYCLE_REPORT = $ReportPath", "& $NodeBin $LifecycleScript", "exit $LASTEXITCODE"]) expect(nativeWorker).toContain(value);
    expect(nativeWorker).not.toMatch(/Password|UserName|SID/iu);
    ...
    expect(standardUserScript).toContain("$owner.Value -ne $createdSid");
    ...
    for (const value of ["scheduled_task/action", "scheduled_task/settings", "scheduled_task/register", "scheduled_task/export", "scheduled_task/verify_logon", "scheduled_task/verify_runlevel", "scheduled_task/verify_password", "scheduled_task/start", "scheduled_task/exit", "scheduled_task/report_missing"]) expect(standardUserScript).toContain(value);
