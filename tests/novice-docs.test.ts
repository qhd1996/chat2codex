...
describe("novice acceptance documentation and Windows CI", () => {
  ...
  test("runs the novice fast, native, 30-run, pack, extraction, and residual gates on Windows", async () => {
    ...
    expect(parsed?.on?.push?.branches).toEqual(["candidate/novice-0.8.0-novice.17"]);
    expect(parsed?.env?.C2C_EXPECTED_CANDIDATE_VERSION).toBe("0.8.0-novice.17");
    expect(parsed?.env?.C2C_EXPECTED_CANDIDATE_SHA256).toBe("d2be7c3ba6ce3cd6373e75664f5e8cdf7391e045425e0ac4e5f662d21e74ef43");
    ...
    for (const value of ["New-LocalUser", "ProcessStartInfo", ".UserName", "novice-native-lifecycle-built.mjs", "Remove-LocalUser", "residualUsers", "residualProcesses", "profileExists", "profile_cleanup", "ownedRootExists"]) expect(standardUserScript).toContain(value);
    expect(standardUserScript).toContain("Join-Path $profilePath ('AppData\\Local\\Temp\\C2C-Native-'");
