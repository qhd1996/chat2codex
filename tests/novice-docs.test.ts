...
describe("novice acceptance documentation and Windows CI", () => {
  ...
  test("runs the novice fast, native, 30-run, pack, extraction, and residual gates on Windows", async () => {
    ...
    expect(parsed?.on?.push?.branches).toEqual(["candidate/novice-0.8.0-novice.26"]);
    expect(parsed?.env?.C2C_EXPECTED_CANDIDATE_VERSION).toBe("0.8.0-novice.26");
    expect(parsed?.env?.C2C_EXPECTED_CANDIDATE_SHA256).toBe("01e4910780ddf20c8820ecd2e1b8fe3cf555bf73a86068334e05f34aa48ccb5a");
