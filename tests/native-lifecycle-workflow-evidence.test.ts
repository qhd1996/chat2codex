...
describe("native lifecycle workflow evidence", () => {
  test("fails closed when a pass report lacks qualifying summary or cleanup", () => {
    ...
    const execution = { identity: "current_runner", logonType: "Interactive", runLevel: "Limited", administrator: false, sidMatched: true, profileMode: "disposable_env" };
    ...
    expect(publicNativeLifecycleEvidence({ schemaVersion: 1, verdict: "pass", summary: null, execution, failure: null, cleanup }, invalid).verdict).toBe("fail");
    ...
    expect(publicNativeLifecycleEvidence({ schemaVersion: 1, verdict: "pass", summary: qualifyingSummary, execution: { ...execution, administrator: true }, failure: null, cleanup }, invalid).verdict).toBe("fail");
    expect(publicNativeLifecycleEvidence({ schemaVersion: 1, verdict: "pass", summary: qualifyingSummary, execution, failure: null, cleanup }, invalid)).toEqual({ schemaVersion: 1, verdict: "pass", execution, failure: null, cleanup: publicCleanup });
  });
