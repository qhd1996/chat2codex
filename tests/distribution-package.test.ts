...
describe("distribution package contract", () => {
  test("validates the repository package inputs and portable release manifest", async () => {
    await expect(validateDistributionTree(repositoryRoot)).resolves.toMatchObject({
      packageVersion: "0.8.0-novice.25", hookCount: 3, forbiddenHits: 0,
    });
  });
...
test("packages every novice acceptance runtime asset and declares Node 20.19 minimum", async () => {
  ...
  for (const asset of [
    ...
    "scripts/novice-service-probe.mjs", "scripts/novice-clean-windows-attestation.mjs", "scripts/novice-process-cleanup.mjs", "scripts/novice-real-upgrade-probe.mjs", "scripts/novice-state-runtime-probe.mjs", "scripts/novice-another-user-acl.ps1",
    "scripts/chat2codex-personal.ps1",
