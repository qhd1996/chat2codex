...
test("requires a hash-bound real old-package upgrade, rollback, uninstall, and reinstall chain", () => {
  ...
  expect(validateRealUpgradeEvidence(value, {
    ...
    oldVersion: "0.8.0-orchestrator.4", candidateVersion: "0.8.0-novice.2",
  ...
  ]) { const candidate = validUpgrade(); mutate(candidate); expect(() => validateRealUpgradeEvidence(candidate, { oldArchiveSha256: "a".repeat(64), candidateArchiveSha256: "b".repeat(64), oldVersion: "0.8.0-orchestrator.4", candidateVersion: "0.8.0-novice.2", ownedEnvironmentHash: "c".repeat(64), runIdentityHash: "d".repeat(64) })).toThrow(/upgrade|rollback|uninstall|reinstall|hash|pending|residual/i); }
});

function validUpgrade() {
  return {
    oldArchiveSha256: "a".repeat(64), candidateArchiveSha256: "b".repeat(64), oldVersion: "0.8.0-orchestrator.4", candidateVersion: "0.8.0-novice.2",
    ...
    finalPackageVersion: "0.8.0-novice.2", userDataPreserved: true, commands: ["npm install <old-archive>", "npm install <candidate-archive>", "npm uninstall chat2codex"], residualProcesses: 0,
  };
}
