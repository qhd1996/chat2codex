...
describe("command actions", () => {
  ...
  test("parses only review-gated advisor decisions and fails invalid decisions into a safe list", () => {
    expect(parseSlashCommand("/advisor approve usage-delivery-retry")).toEqual({
      kind: "review_advisor_proposal",
      proposalId: "usage-delivery-retry",
      decision: "approve_for_planning",
    });
    expect(parseSlashCommand("/advisor reject usage-delivery-retry")).toEqual({
      kind: "review_advisor_proposal",
      proposalId: "usage-delivery-retry",
      decision: "reject",
    });
    expect(parseSlashCommand("/advisor apply usage-delivery-retry")).toEqual({ kind: "list_advisor_proposals" });
    expect(parseSlashCommand("/advisor approve")).toEqual({ kind: "list_advisor_proposals" });
  });
});
