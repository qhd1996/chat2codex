...
export class ImageDraftService {
  ...
  private async validateFile(filePath: string, mediaType: string) {
    ...
    const bytes = await fs.readFile(real); if (!signatureMatches(bytes, mediaType)) throw new Error("Image signature does not match media type.");
    return { path: real, bytes: stat.size, sha256: createHash("sha256").update(bytes).digest("hex") };
  }
...
function signatureMatches(bytes: Buffer, mediaType: string): boolean {
