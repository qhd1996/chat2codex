...
export interface PortableReceiptV1 {
  ...
  prerequisites: PortablePrerequisiteSnapshot; backup: PortableBackupReference | null; installedHashes: PortableOwnedHashes | null; pendingStep: PortableInstallStep | null; completed: PortableInstallStep[];
...
export function parsePortableReceipt(value: unknown): PortableReceiptV1 {
  ...
  const installedHashes = value.installedHashes === null ? null : parseHashes(value.installedHashes);
  validateState(value.status as PortableReceiptStatus, completed, pendingStep, backup, installedHashes, rollbackCompleted, rollbackFailures);
  return { schemaVersion: 1, receiptId: value.receiptId, action: value.action as PortableReceiptAction, status: value.status as PortableReceiptStatus, ...(value.archiveSha256 ? { archiveSha256: value.archiveSha256 } : {}), home, npmPrefix, receiptRoot, createdAt: value.createdAt as string, updatedAt: value.updatedAt as string, prerequisites, backup, installedHashes, pendingStep, completed, rollbackCompleted, rollbackFailures };
}
export function parsePortableUninstallManifest(value: unknown): PortableUninstallManifestV1 {
  object(value); exact(value, ["home", "npmPrefix", "ownedPackageFiles", "schemaVersion"]);
  if (value.schemaVersion !== 1) throw new Error("Portable uninstall manifest schema is unsupported.");
  const home = absolute(String(value.home)); const npmPrefix = absolute(String(value.npmPrefix)); inside(npmPrefix, home);
  if (!Array.isArray(value.ownedPackageFiles) || value.ownedPackageFiles.length === 0 || value.ownedPackageFiles.length > 10_000 || value.ownedPackageFiles.some((item) => typeof item !== "string")) throw new Error("Portable uninstall manifest owned files are invalid.");
  const ownedPackageFiles = value.ownedPackageFiles.map((item) => absolute(item));
  for (const filePath of ownedPackageFiles) { inside(filePath, npmPrefix); if (path.win32.relative(npmPrefix, filePath) === "") throw new Error("Portable uninstall manifest cannot own the npm prefix root."); }
  if (new Set(ownedPackageFiles.map((item) => item.toLocaleLowerCase())).size !== ownedPackageFiles.length) throw new Error("Portable uninstall manifest contains duplicate paths.");
  return { schemaVersion: 1, home, npmPrefix, ownedPackageFiles };
}
...
function parseHashes(value: unknown): PortableOwnedHashes {
  ...
  return { package: hashOrNull(hashes.package), config: hashOrNull(hashes.config), state: hashOrNull(hashes.state), task: hashOrNull(hashes.task) };
}
...
function absolute(value: string): string { if (!path.win32.isAbsolute(value) || /[\u0000-\u001f]/u.test(value)) throw new Error("Portable receipt path is invalid."); return path.win32.normalize(value); }
...
function validateState(status: PortableReceiptStatus, completed: PortableInstallStep[], pending: PortableInstallStep | null, backup: PortableBackupReference | null, installedHashes: PortableOwnedHashes | null, rollbackCompleted: PortableRollbackStep[], rollbackFailures: PortableRollbackStep[]): void {
