...
interface NativeContext { plan: PersonalPortablePlan; options: Required<PersonalPortableNativeOptions>; serviceInput: WindowsServiceInstallInput; serviceIo: ReturnType<typeof createWindowsServiceIo>; currentBackup?: string; expectedBackup?: PortableBackupReference; restoreWasOnline?: boolean; activeReceiptId?: string; }
...
function installIo(context: NativeContext) {
  ...
  const base = {
    now: () => new Date(), receiptId: () => { const id = `receipt-${Date.now()}-${randomUUID().slice(0, 8)}`; context.activeReceiptId = id; return id; }, archiveSha256: sha256File,
